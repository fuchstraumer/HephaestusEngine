...
static uint32_t morton_encode(const T x, const T y, const T z) {
    ...
    if constexpr (std::is_same_v<T, uint32_t>) {
        return _pdep_u32(z, BMI_X_MASK<T>) | _pdep_u32(y, BMI_Y_MASK<T>) | _pdep_u32(x, BMI_Z_MASK<T>);
    }
    else if constexpr (std::is_same_v<T, uint64_t>) {
        return _pdep_u64(z, BMI_X_MASK<T>) | _pdep_u64(y, BMI_Y_MASK<T>) | _pdep_u64(x, BMI_Z_MASK<T>);
    }
}
...
static void morton_decode(const T input_code, T& result_x, T& result_y, T& result_z) {
    ...
    if constexpr (std::is_same_v<T, uint32_t>) {
        result_x = _pext_u32(input_code, BMI_X_MASK<T>);
        result_y = _pext_u32(input_code, BMI_Y_MASK<T>);
        result_z = _pext_u32(input_code, BMI_Z_MASK<T>);
    }
    else if constexpr (std::is_same_v<T, uint64_t>) {
        result_x = _pext_u64(input_code, BMI_X_MASK<T>);
        result_y = _pext_u64(input_code, BMI_Y_MASK<T>);
        result_z = _pext_u64(input_code, BMI_Z_MASK<T>);
    }
}
