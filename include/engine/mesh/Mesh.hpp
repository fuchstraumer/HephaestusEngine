...
#include "stdafx.h"
#include "engine/mesh/MeshComponents.hpp"
...
namespace mesh {
	...
	struct BlockVertices {
		std::vector<glm::vec3> positions;
		std::vector<block_vert_data> normals_uvs;
		...
		void push_back(block_vertex_t vert) {
			positions.push_back(std::move(vert.Position));
			normals_uvs.push_back(std::move(block_vert_data{ std::move(vert.Normal), std::move(vert.UV) }));
		}
		...
		size_t DataSize() const noexcept {
			return normals_uvs.size() * sizeof(block_vert_data);
		}
